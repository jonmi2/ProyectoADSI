...
{	
	        ...
	        if (conn != null)
	        {
	            ...
	            if (resultadoUsuario.next()) 
	            {
	                ...
	                if (resultadoPelicula.next()) 
	                {
	                    ...
	                    // Si ambas tablas est�n vac�as, insertar los datos iniciales
	                    if (usuarioCount == 0 && peliculaCount == 0) 
	                    {
	                        // Insertar un usuario con rol admin
	                        String insertAdmin = "INSERT INTO Usuario (nombre, email, rol, aceptadoPor) VALUES ('Juan P�rez', 'juan@example.com', 'admin', NULL)";
	                        execSQL(insertAdmin);
	                        ...
	                        String insertUsuarios = "INSERT INTO Usuario (nombre, email, rol, aceptadoPor) "
	                                + "VALUES ('Mar�a L�pez', 'maria@example.com', 'usuario registrado', 1), "
	                                + "('Carlos Garc�a', 'carlos@example.com', 'usuario registrado', 1)";
	                        execSQL(insertUsuarios);

	                        // Insertar pel�culas con el idAceptador del admin (idUsuario = 1)
	...
	// M�todo que verifica si el admin ya existe en la base de datos
